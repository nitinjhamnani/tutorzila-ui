...
import { SearchIcon, XIcon, BookOpen, Users, MapPin, Filter, ListFilter } from "lucide-react";
...
const MOCK_REQUIREMENTS: TuitionRequirement[] = [
  ...
  { id: "3", parentId: "p3", parentName: "Carol Williams", subject: "English", gradeLevel: "Grade 6-8", scheduleDetails: "Tues, Thurs 4-6 PM", location: "Online", status: "open", postedAt: new Date(Date.now() - 86400000 * 1).toISOString(), additionalNotes: "Focus on grammar and essay writing." },
  ...
  { id: "5", parentId: "p5", parentName: "Eve Davis", subject: "Mathematics", gradeLevel: "Grade 1-5", scheduleDetails: "Sat 10 AM - 12 PM", location: "Tutor's Home", status: "open", postedAt: new Date(Date.now() - 86400000 * 3).toISOString() },
  { id: "6", parentId: "p6", parentName: "Frank Green", subject: "Chemistry", gradeLevel: "Grade 9-10", scheduleDetails: "Mon 7-9 PM", location: "Online", status: "open", postedAt: new Date(Date.now() - 86400000 * 1).toISOString() },
  { id: "7", parentId: "p7", parentName: "Grace Hall", subject: "Biology", gradeLevel: "Grade 11-12", scheduleDetails: "Flexible Evening Hours", location: "Student's Home", status: "open", postedAt: new Date(Date.now() - 86400000 * 6).toISOString(), additionalNotes: "Looking for an experienced biology tutor for IB curriculum." },
];
...
export default function AllEnquiriesPage() {
  ...
  useEffect(() => {
    // Simulate API call - in a real app, fetch only open requirements
  ...
  return (
    <div className={`space-y-8 ${containerPadding} py-8`}>
      <Card className="shadow-xl bg-gradient-to-br from-primary/15 via-card to-card border-none animate-in fade-in duration-700 ease-out rounded-xl overflow-hidden">
        <CardHeader className="p-6 md:p-8">
          ...
          <CardDescription className="text-lg md:text-xl text-foreground/80 mt-1">
            Browse and apply for available tuition requirements.
          </CardDescription>
        </CardHeader>
