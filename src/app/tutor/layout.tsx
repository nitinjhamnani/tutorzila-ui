...
import {
  ...
  LogOut, PanelLeft, // Added PanelLeft for the collapse icon
 Menu as MenuIcon
} from "lucide-react";
...
export default function TutorSpecificLayout({ children }: { children: ReactNode }) {
  ...
  // State and function for sidebar collapse moved from TutorSidebar
  const [isCollapsed, setIsCollapsed] = useState(false);
  const [openMobile, setOpenMobile] = useState(false); // New state for mobile sidebar open/close
  const toggleCollapse = () => {
    console.log('toggleCollapse called:', { isMobile, isCollapsed, openMobile });
    if (isMobile) {
      ...
      {hasMounted && (
        <header
          className={cn(
            ...
            "sticky top-0" // Sticks to top-0 directly
          )}
        >
          <div className="flex items-center justify-between w-full">
            <div className="flex items-center gap-2">
              {/* Sidebar Toggle Button */}
              <Button variant="ghost" size="icon" onClick={toggleCollapse} aria-label="Toggle Sidebar">
                {/* You can conditionally render an icon here based on isCollapsed if needed */}
 ...
 toggleCollapse={toggleCollapse} />)} {/* Pass the toggle function */}
        <main
          className={cn( // Removed top padding
            ...
            "px-4 md:px-6" // Standard padding for the content area itself
