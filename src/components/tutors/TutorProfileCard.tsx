...
const InfoItem = ({ icon: Icon, text, className }: { icon: React.ElementType; text?: string | string[]; className?: string }) => {
  ...
  return (
    <div className={cn("flex items-center text-[11px] text-muted-foreground group-hover:text-foreground/80 transition-colors", className)}>
    ...
    <Link href={`/tutors/${tutor.id}`} passHref legacyBehavior>
      <a className="block group cursor-pointer h-full"> {/* Ensure the anchor tag takes full height */}
        <Card className={cn(
          ...
          "flex flex-col p-4", // Increased padding for more breathing room
          ...
          <CardHeader className="flex flex-row items-center gap-3 p-0 mb-3">
            <Avatar className="w-14 h-14 border-2 border-primary/20 shadow-sm shrink-0"> {/* Slightly larger avatar */}
            ...
            <div className="flex-1 min-w-0">
              ...
              <div className="flex items-center text-xs text-muted-foreground mt-0.5 group-hover:text-foreground/80 transition-colors">
                <Star className="w-3.5 h-3.5 fill-amber-400 text-amber-400 mr-1" />
                <span>{typeof rating === 'number' ? rating.toFixed(1) : 'N/A'} ({mockReviewCount} reviews)</span>
              </div>
            </div>
          </CardHeader>

          <CardContent className="p-0 space-y-2 flex-grow"> {/* Increased spacing */}
            <InfoItem icon={BookOpen} text={tutor.subjects} />
            {tutor.grade && <InfoItem icon={GraduationCap} text={tutor.grade} />}
            <InfoItem icon={TeachingModeIcon} text={teachingModeText} />
          ...
          <CardFooter className="p-0 mt-3 pt-3 border-t border-border/20 flex justify-end items-center">
            {tutor.hourlyRate && (
              <Badge variant="outline" className="text-[11px] py-1 px-2.5 border-primary/40 bg-primary/10 text-primary group-hover:bg-primary group-hover:text-primary-foreground transition-colors">
